...
(comment
  ...
  (shrink-with (constantly true) 1)

  (last (map value (shrinkables-with (constantly true) 1)))
  
  (defn pessimistic-pred [expected]
    (fn [x]
      (if (= x expected)
        true
        false)))
  ...
  (with-factories (select-factories [])
    (shrink-with ok? data))
